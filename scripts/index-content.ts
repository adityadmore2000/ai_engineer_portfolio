...
async function fetchFromSanity() {
  ...
  return Promise.all([
    client.fetch<SanityProject[]>(
      groq`*[_type == "project" && published == true] { _id, title, "slug": slug.current, shortSummary, status, whyIBuiltIt, theProblem, theSolution, engineeringDecisions, results, whatThisDemonstrates, exampleInputsOutputs, lessonsLearned, limitations, futureImprovements, timeline, technologies, keyMetrics }`
    ),
